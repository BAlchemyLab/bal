...
curl $NODE0/nodes
echo curl -s -X POST -H "Content-Type: application/json" -d '{ "nodes": ["'$NODE1'"] }' $NODE0/nodes/register
curl -s -X POST -H "Content-Type: application/json" -d '{ "nodes": ["'$NODE1'"] }' $NODE0/nodes/register
echo curl $NODE0/nodes
curl $NODE0/nodes
